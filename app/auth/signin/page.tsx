...
const SignIn = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // TODO: Integrate with backend signIn
      // For demo, store user name/email in localStorage
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen w-full font-sans bg-[#F1ECFF]">
      ...
      <div className={`w-full md:w-1/2 flex justify-center items-center px-4 sm:px-10 lg:px-20 py-6 md:py-10 transition-all duration-700 ease-out pt-24 md:pt-10 ${animatePanel ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-10"}`}>
        <form onSubmit={handleSubmit} className="w-full max-w-md space-y-4 sm:space-y-6" autoComplete="on">
          ...
          <input type="email" placeholder="Your Email" ref={emailRef} name="email" autoComplete="username" className="w-full p-2.5 sm:p-3 border-2 border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-[#6A4EFF] text-sm transition-all duration-300 focus:scale-[1.02] hover:border-[#B8AAFF]" required />
          <div className="relative">
            <input type={showPassword ? "text" : "password"} placeholder="Enter Password" ref={passwordRef} name="password" autoComplete="current-password" className="w-full p-2.5 sm:p-3 border-2 border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-[#6A4EFF] text-sm pr-10 transition-all duration-300 focus:scale-[1.02] hover:border-[#B8AAFF]" required />
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center text-xs sm:text-sm">
            ...
            <button type="button" onClick={() => router.push("/auth/forgot-password")} className="text-[#6356D7] hover:underline font-bold">Forgot password?</button>
          </div>
          <button type="submit" disabled={isLoading} className="w-full py-2.5 sm:py-3 bg-[#6356D7] text-white rounded-md hover:bg-[#7E5FFF] font-semibold transition-all text-sm shadow-md">{isLoading ? "Signing In..." : "Sign In"}</button>
        </form>
      </div>
